...
int	sl_keyinput(int keycode, t_data *data)
{
	if (keycode == KEY_A && data->y > 0 && data->wk > 5 && data->end == 0 && \
		data->map[data->x][data->y - 1] != 'E' && data->map[data->x][data->y - 1] != '1')
		sl_key_manage(data, keycode);
	else if (keycode == KEY_S && data->x < sl_checklen(data->map, NULL, 1) - 1 && \
		data->wk > 5 && data->end ==0 && data->map[data->x + 1][data->y] != 'E' && data->map[data->x + 1][data->y] != '1')
		sl_key_manage(data, keycode);
	else if (keycode == KEY_D && data->y < sl_checklen(NULL, data->map[0], 2) - 1 && \
		data->wk > 5 && data->end == 0 && data->map[data->x][data->y + 1] != 'E' && data->map[data->x][data->y + 1] != '1')
		sl_key_manage(data, keycode);
	else if (keycode == KEY_W && data->x > 0 && data->wk > 5 && data->end == 0 && \
		data->map[data->x - 1][data->y] != 'E' && data->map[data->x - 1][data->y] != '1')
		sl_key_manage(data, keycode);
